...
# ==========================================
# 🧪 가상 체결 시뮬레이터 (Virtual Test Mode)
# ==========================================
with st.expander("🧪 가상 매수/매도 체결 테스트 시뮬레이터 (장 마감 시간 테스트용)"):
    st.info("💡 장 마감 시간이나 모의투자에서도 실제 매수가 진행된 상황을 가정하여 회차 진행, 평단가/보유수량 변화, 깃허브 DB 저장을 미리 시뮬레이션할 수 있습니다.")
    
    sim_col1, sim_col2 = st.columns(2)
    with sim_col1:
        if st.button("🧪 [1순위 평단 LOC] 가상 체결 (+1회차)", use_container_width=True):
            if buy1_qty > 0 and buy1_price > 0:
                old_shares = float(project_data.get("total_shares", 0.0))
                old_spent = float(project_data.get("total_spent", 0.0))
                
                new_shares = old_shares + buy1_qty
                new_spent = old_spent + (buy1_qty * buy1_price)
                new_avg = new_spent / new_shares if new_shares > 0 else 0.0
                
                project_data["turn"] = turn + 1
                project_data["total_shares"] = new_shares
                project_data["total_spent"] = new_spent
                project_data["avg_price"] = new_avg
                
                sim_log = {
                    "date": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S (가상)"),
                    "env": f"{api.env}_SIMULATION",
                    "target": target_etf,
                    "turn_before": turn,
                    "turn_after": project_data["turn"],
                    "orders": [{"구분": "가상 매수 1순위", "수량": buy1_qty, "단가": buy1_price}]
                }
                project_data.setdefault("history", []).append(sim_log)
                state["projects"][active_id] = project_data
                _, sha = db.update_state(state, sha)
                st.success(f"🎉 1순위 가상 체결 완료! (회차: {project_data['turn']}회 | 평단가: ${new_avg:.2f} | 수량: {new_shares}주)")
                time.sleep(1)
                st.rerun()
            else:
                st.warning("매수 가능 수량이 0주입니다.")

        if st.button("🧪 [1순위+2순위 2분할] 가상 체결 (+1회차)", use_container_width=True):
            tot_qty = buy1_qty + buy2_qty
            if tot_qty > 0:
                old_shares = float(project_data.get("total_shares", 0.0))
                old_spent = float(project_data.get("total_spent", 0.0))
                
                added_spent = (buy1_qty * buy1_price) + (buy2_qty * buy2_price)
                new_shares = old_shares + tot_qty
                new_spent = old_spent + added_spent
                new_avg = new_spent / new_shares if new_shares > 0 else 0.0
                ...
                sim_log = {
                    ...
                    "turn_after": project_data["turn"],
                    "orders": [
                        {"구분": "가상 매수 1순위", "수량": buy1_qty, "단가": buy1_price},
                        {"구분": "가상 매수 2순위", "수량": buy2_qty, "단가": buy2_price}
                    ]
                }
                project_data.setdefault("history", []).append(sim_log)
                state["projects"][active_id] = project_data
                _, sha = db.update_state(state, sha)
                st.success(f"🎉 2분할 가상 체결 완료! (회차: {project_data['turn']}회 | 평단가: ${new_avg:.2f} | 수량: {new_shares}주)")
                time.sleep(1)
                st.rerun()

    with sim_col2:
        if st.button("🎉 [익절 매도] 가상 체결 (원금상환 & 0회차 리셋)", use_container_width=True):
            if db_shares > 0:
                realized_pnl = (sell_price - db_avg_price) * db_shares
                sim_log = {
                    "date": pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S (가상)"),
                    "env": f"{api.env}_SIMULATION",
                    "target": target_etf,
                    "turn_before": turn,
                    "turn_after": 0,
                    "orders": [{"구분": "가상 익절 매도", "수량": db_shares, "단가": sell_price, "수익금": f"+${realized_pnl:.2f}"}]
                }
                project_data["turn"] = 0
                project_data["total_shares"] = 0.0
                project_data["total_spent"] = 0.0
                project_data["avg_price"] = 0.0
                project_data.setdefault("history", []).append(sim_log)
                state["projects"][active_id] = project_data
                _, sha = db.update_state(state, sha)
                st.success(f"🏆 익절 매도 성공! 실현 수익: +${realized_pnl:.2f} (0회차 초기화 완료)")
                time.sleep(1.5)
                st.rerun()
            else:
                st.warning("매도할 보유 수량이 없습니다.")

        if st.button("🔄 [시뮬레이션 & 히스토리 전체 초기화]", use_container_width=True):
            project_data["turn"] = 0
            project_data["total_shares"] = 0.0
            project_data["total_spent"] = 0.0
            project_data["avg_price"] = 0.0
            ...
            state["projects"][active_id] = project_data
            _, sha = db.update_state(state, sha)
            st.success("시뮬레이션 데이터 및 히스토리 전체가 0회차로 깔끔히 초기화되었습니다.")
            st.rerun()

# ==========================================
# 📜 사이클 매매 기록 타임라인 (History Timeline)
# ==========================================
hist_list = project_data.get("history", [])
if hist_list:
    with st.expander(f"📜 [{project_data['name']}] 사이클 매매 및 시뮬레이션 히스토리 ({len(hist_list)}건)"):
        if st.button("🗑️ 히스토리 내역 삭제", key="clear_hist_btn"):
            project_data["history"] = []
            state["projects"][active_id] = project_data
            _, sha = db.update_state(state, sha)
        ...
        st.markdown("---")
        for idx, log in enumerate(reversed(hist_list)):
            st.markdown(f"**📅 {log.get('date')}** | 전후 회차: `{log.get('turn_before')}회` ➔ `{log.get('turn_after')}회`")
            for ord_item in log.get("orders", []):
                price_val = float(ord_item.get('단가', 0.0))
                st.text(f"  • {ord_item.get('구분')}: {ord_item.get('수량')}주 @ ${price_val:.2f}")
            if idx < len(hist_list) - 1:
                st.markdown("---")
